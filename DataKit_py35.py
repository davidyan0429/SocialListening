        ...
        for long in range(len(predict_label_class2_1)):
            ...
            for i in idList[long]:
                for item in sent_result:
                    tmp_list = [i]
                    tmp_list.append(sub_result['class1'])
                    tmp_list.append(sub_result['class2'])
                    tmp_list.append(sub_result['sentence'])
                    tmp_list.append(sub_result['sentiment'])
...
if __name__=="__main__":
    ...
    while (now > start):
        ...
        try:
            ...
            dataKit.conn.commit()
            dataKit.cur.execute("SELECT top %s id from rawdata order by id desc" % numberOfdata)
